...
static void leme_output_handle_frame(struct wl_listener *listener, void *data) {
  struct leme_output *output = wl_container_of(listener, output, frame);
...
static struct leme_output *leme_output_find(struct leme_server *server,
                                            const char *name) {
  struct leme_output *output;
...
static bool leme_output_logical_size(
  ...
  if (physical_width <= 0 || physical_height <= 0 || !isfinite(scale) ||
      scale <= 0.0) {
    return false;
  }
  ...
  if (!isfinite(logical_width) || !isfinite(logical_height) ||
      logical_width < 1.0 || logical_width > INT_MAX || logical_height < 1.0 ||
      logical_height > INT_MAX) {
    return false;
  }
...
static size_t leme_output_count(const struct leme_server *server) {
...
static bool leme_output_build_plans(struct leme_server *server,
                                    const struct leme_config *config,
                                    bool startup,
...
static bool leme_output_resolve_plans(struct leme_output_plan *plans,
                                      size_t count) {
  struct leme_placement_request *requests = calloc(count, sizeof(*requests));
  ...
  for (index = 0; index < count; index++) {
    ...
    if (!leme_output_logical_size(
            ...
            leme_output_transform(plans[index].entry.transform), &width,
            &height)) {
      free(requests);
      free(placements);
      return false;
...
static bool
leme_output_head_box(const struct wlr_output_configuration_head_v1 *head,
                     struct wlr_box *box);
  ...
  *used_mode_fallback = false;
  ...
  for (index = 0; index < count; index++) {
    ...
    struct wlr_output_configuration_head_v1 *head =
        wlr_output_configuration_head_v1_create(configuration,
                                                plan->output->wlr_output);
    ...
    if (plan->enabled) {
      ...
      head->state.transform = leme_output_transform(plan->entry.transform);
    }
  }
...
static bool leme_output_build_power_on_plan(struct leme_output *target,
...
static bool leme_output_box_edges(
  ...
  if (box_right < INT_MIN || box_right > INT_MAX || box_bottom < INT_MIN ||
      box_bottom > INT_MAX) {
    return false;
  }
  if (right != NULL) {
...
static bool leme_output_plan_box(const struct leme_output_plan *plan,
                                 struct wlr_box *box) {
  *box = (struct wlr_box){
      .x = plan->x,
      .y = plan->y,
  ...
  return leme_output_logical_size(
             ...
             plan->entry.scale, leme_output_transform(plan->entry.transform),
             &box->width, &box->height) &&
         leme_output_box_edges(box, NULL, NULL);
}

static bool leme_output_boxes_overlap(const struct wlr_box *first,
                                      const struct wlr_box *second) {
  int64_t first_right;
  ...
  return (int64_t)first->x < second_right && (int64_t)second->x < first_right &&
         (int64_t)first->y < second_bottom && (int64_t)second->y < first_bottom;
}

static bool leme_output_boxes_fit_layout(const struct wlr_box *first,
                                         const struct wlr_box *second) {
  int64_t first_right;
  int64_t first_bottom;
  int64_t second_right;
...
static bool leme_output_power_plan_overlaps(struct leme_output *target,
                                            const struct wlr_box *target_box,
                                            const char **conflict) {
...
static void leme_output_states_finish(struct wlr_backend_output_state *states,
                                      size_t states_len) {
  size_t index;
...
static bool leme_output_states_prepare(struct leme_server *server,
                                       struct wlr_backend_output_state *states,
                                       size_t states_len, bool testing) {
  size_t index;

  for (index = 0; index < states_len; index++) {
    struct leme_output *output =
        leme_output_from_wlr(server, states[index].output);
...
static bool leme_output_head_changes_lifecycle(
  ...
  if (head->state.x != output->layout_x || head->state.y != output->layout_y ||
...
static void leme_output_finish_reconfigured_animations(
  ...
  wl_list_for_each(head, &configuration->heads, link) {
    ...
    if (output != NULL && leme_output_head_changes_lifecycle(head, output)) {
      leme_render_output_animations_finish(output);
    }
  }
}

static bool leme_output_configuration_snapshots(
    struct leme_server *server,
    const struct wlr_output_configuration_v1 *configuration,
  ...
  if (snapshots == NULL || *snapshots != NULL || snapshot_count == NULL ||
      count == 0 || count > SIZE_MAX / sizeof(*prepared)) {
    return false;
  }
...
leme_output_configuration_run(struct leme_server *server,
  ...
  states = wlr_output_configuration_v1_build_state(configuration, &states_len);
  if (states == NULL) {
    return false;
  }
  valid = leme_output_states_prepare(server, states, states_len, true) &&
  ...
  valid = leme_output_states_prepare(server, states, states_len, false) &&
          wlr_backend_commit(server->backend, states, states_len);
  leme_output_states_finish(states, states_len);
  return valid;
}

static bool leme_output_configuration_matches_current(
    struct leme_server *server,
    const struct wlr_output_configuration_v1 *configuration) {
  struct wlr_output_configuration_head_v1 *head;
  ...
  wl_list_for_each(head, &configuration->heads, link) {
    ...
    const struct leme_output *output = leme_output_from_wlr(server, wlr_output);
    const bool adaptive =
        wlr_output->adaptive_sync_status == WLR_OUTPUT_ADAPTIVE_SYNC_ENABLED;

    if (output == NULL || head->state.enabled != wlr_output->enabled ||
...
static bool leme_output_configuration_test(
    struct leme_server *server,
    struct wlr_output_configuration_v1 *configuration) {
  return leme_output_configuration_run(server, configuration, false);
}

static bool leme_output_configuration_commit(
    struct leme_server *server,
    struct wlr_output_configuration_v1 *configuration) {
  return leme_output_configuration_run(server, configuration, true);
}

static bool leme_output_reconcile(struct leme_server *server,
...
static void leme_output_track_configuration(
    struct leme_server *server,
    const struct wlr_output_configuration_v1 *configuration) {
  struct wlr_output_configuration_head_v1 *head;
...
static void leme_output_detach_all_scenes(struct leme_server *server) {
  struct leme_output *output;
...
static bool leme_output_commit_and_reconcile(
    struct leme_server *server,
    struct wlr_output_configuration_v1 *configuration) {
  ...
  if (previous == NULL ||
      !leme_output_configuration_snapshots(server, configuration, &snapshots,
                                           &snapshot_count) ||
      !leme_sticky_prepare_outputs(server, snapshots, snapshot_count,
                                   &sticky)) {
    free(snapshots);
...
static void leme_output_power_warp_focus(struct leme_server *server,
                                         const struct leme_output *previous) {
  struct leme_box box;

  if (server->focused_output == previous || server->focused_output == NULL ||
      server->cursor == NULL || server->config == NULL ||
...
bool leme_output_set_power(struct leme_output *output, bool on) {
  ...
  if (output == NULL || output->server == NULL || output->wlr_output == NULL) {
    return false;
  }
  server = output->server;
  name =
      output->wlr_output->name == NULL ? "unknown" : output->wlr_output->name;
  ...
  if (on && !leme_output_build_power_on_plan(output, &plan)) {
    wlr_log(WLR_ERROR, "leme: cannot build power-on plan for output %s", name);
    return false;
  }
  if (on && plan.mode == NULL &&
      (output->wlr_output->width <= 0 || output->wlr_output->height <= 0)) {
    wlr_log(WLR_ERROR, "leme: output %s has no available power-on mode", name);
    return false;
  }
  if (on && !leme_output_plan_box(&plan, &target_box)) {
    wlr_log(WLR_ERROR, "leme: output %s has unsafe power-on geometry", name);
    return false;
  }
  if (on && leme_output_power_plan_overlaps(output, &target_box, &conflict)) {
    wlr_log(WLR_ERROR,
            "leme: output %s power-on conflicts with enabled output %s", name,
            conflict == NULL ? "unknown" : conflict);
    return false;
  }
  ...
  if (on) {
    if (plan.mode != NULL) {
      ...
      wlr_output_state_set_custom_mode(&state, output->wlr_output->width,
  ...
  if (snapshot_count == 0 || snapshot_count > SIZE_MAX / sizeof(*snapshots)) {
  ...
  wl_list_for_each(candidate, &server->outputs, link) {
    const bool usable =
        candidate == output ? on : candidate->wlr_output->enabled;
    const struct leme_box box =
        candidate == output && on ? wake_box : leme_output_full_box(candidate);

    snapshots[snapshot_index++] = (struct leme_sticky_output_snapshot){
        .output = candidate,
        .future_full = box,
        .usable = usable,
    };
  }
  if (!leme_sticky_prepare_outputs(server, snapshots, snapshot_count,
                                   &sticky)) {
...
static bool
leme_output_request_box(const struct wlr_output_configuration_v1 *configuration,
                        const struct leme_output *output, bool *enabled,
                        struct wlr_box *box) {
  const struct wlr_output_configuration_head_v1 *head =
      leme_output_configuration_head_for(configuration, output->wlr_output);
...
static bool leme_output_request_layout_valid(
  ...
  wl_list_for_each(output, &server->outputs, link) {
    ...
    if (!leme_output_request_box(configuration, output, &first_enabled,
                                 &first)) {
      return false;
    }
    if (!first_enabled) {
    ...
    wl_list_for_each(other, &server->outputs, link) {
      ...
      if (!leme_output_request_box(configuration, other, &second_enabled,
                                   &second)) {
        return false;
      }
      if (second_enabled && (!leme_output_boxes_fit_layout(&first, &second) ||
                             leme_output_boxes_overlap(&first, &second))) {
        return false;
      }
    }
...
static bool leme_output_request_valid(
    struct leme_server *server,
    const struct wlr_output_configuration_v1 *configuration) {
  struct wlr_output_configuration_head_v1 *head;
  ...
  wl_list_for_each(head, &configuration->heads, link) {
    ...
    if (output == NULL || !isfinite(head->state.scale) ||
    ...
    if (head->state.enabled &&
        (head->state.mode == NULL ||
         !leme_output_mode_is_advertised(wlr_output, head->state.mode))) {
      return false;
    }
  }
  return enabled >= 1 &&
...
static void leme_output_handle_manager_test(struct wl_listener *listener,
                                            void *data) {
  struct leme_server *server =
...
static void leme_output_handle_manager_apply(struct wl_listener *listener,
                                             void *data) {
  struct leme_server *server =
...
static void leme_output_handle_commit(struct wl_listener *listener,
                                      void *data) {
  struct leme_output *output = wl_container_of(listener, output, commit);
  const struct wlr_output_event_commit *event = data;
  const uint32_t geometry_fields =
...
static void leme_output_ensure_fallback(struct leme_server *server) {
  ...
  if (server->headless_backend == NULL) {
    server->headless_backend =
        wlr_headless_backend_create(wl_display_get_event_loop(server->display));
    if (server->headless_backend == NULL) {
      wlr_log(WLR_ERROR, "%s",
              "leme: failed to create the fallback headless backend");
      return;
    }
    if (!wlr_multi_backend_add(server->backend, server->headless_backend)) {
      wlr_log(WLR_ERROR, "%s",
    ...
    if (server->started && !wlr_backend_start(server->headless_backend)) {
      wlr_log(WLR_ERROR, "%s",
              "leme: failed to start the fallback headless backend");
      wlr_multi_backend_remove(server->backend, server->headless_backend);
      wlr_backend_destroy(server->headless_backend);
      server->headless_backend = NULL;
      return;
    }
  }
  if (wlr_headless_add_output(server->headless_backend, 1920, 1080) == NULL) {
    wlr_log(WLR_ERROR, "%s",
...
static void leme_output_retire_fallback(struct leme_server *server) {
  struct leme_output *output;
...
static void leme_output_handle_destroy(struct wl_listener *listener,
                                       void *data) {
  struct leme_output *output = wl_container_of(listener, output, destroy);
...
static bool leme_output_init_tags(struct leme_output *output) {
...
static void leme_output_handle_new(struct wl_listener *listener, void *data) {
  struct leme_server *server = wl_container_of(listener, server, new_output);
  struct wlr_output *wlr_output = data;
  struct leme_output *output;

  if (!wlr_output_init_render(wlr_output, server->allocator,
                              server->renderer)) {
    wlr_log(WLR_ERROR, "leme: failed to initialize output %s",
            wlr_output->name);
    return;
  }
...
bool leme_output_init(struct leme_server *server) {
  ...
  if (server->xdg_output_manager == NULL || server->output_manager == NULL) {
    return false;
  }
...
static bool leme_output_boxes_equal(struct leme_box left,
                                    struct leme_box right) {
...
void leme_output_refresh_geometry(struct leme_output *output) {
  ...
  if (wlr_box_empty(&box)) {
    box.x = 0;
    box.y = 0;
    wlr_output_effective_resolution(output->wlr_output, &box.width,
                                    &box.height);
  }
...
leme_output_from_wlr_output(struct leme_server *server,
                            const struct wlr_output *wlr_output) {
  if (server == NULL || wlr_output == NULL || server->outputs.next == NULL) {
    return NULL;
  }
...
struct leme_output *leme_output_at(struct leme_server *server, double layout_x,
  ...
  wlr_output =
      wlr_output_layout_output_at(server->output_layout, layout_x, layout_y);
...
struct leme_output *leme_output_adjacent(struct leme_server *server,
  ...
  if (server == NULL || origin == NULL || direction < LEME_DIRECTION_LEFT ||
      direction > LEME_DIRECTION_DOWN) {
    return NULL;
  }
  box = leme_output_full_box(origin);
  adjacent = wlr_output_layout_adjacent_output(
      server->output_layout, directions[direction], origin->wlr_output,
...
void leme_output_set_focused(struct leme_server *server,
  ...
  if (server == NULL || output == NULL || !output->wlr_output->enabled) {
    return;
  }
