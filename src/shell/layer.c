...
bool leme_layer_keyboard_is_exclusive(const struct leme_server *server) {
  return server->focused_layer != NULL && server->focused_layer->mapped &&
         server->focused_layer->output != NULL &&
         server->focused_layer->output->wlr_output->enabled &&
         server->focused_layer->wlr_layer_surface->current
...
void leme_layer_focus_on_button(struct leme_layer_surface *layer) {
  if (layer != NULL && layer->wlr_layer_surface->current.keyboard_interactive !=
                           ZWLR_LAYER_SURFACE_V1_KEYBOARD_INTERACTIVITY_NONE) {
...
static void leme_layer_handle_commit(struct wl_listener *listener, void *data) {
  struct leme_layer_surface *layer = wl_container_of(listener, layer, commit);
...
static void leme_layer_handle_map(struct wl_listener *listener, void *data) {
...
static void leme_layer_handle_unmap(struct wl_listener *listener, void *data) {
...
static void leme_layer_popup_finish(struct leme_layer_popup *popup) {
...
static void leme_layer_popup_handle_commit(struct wl_listener *listener,
                                           void *data) {
...
static void leme_layer_popup_handle_reposition(struct wl_listener *listener,
                                               void *data) {
  struct leme_layer_popup *popup = wl_container_of(listener, popup, reposition);

  (void)data;
  leme_render_layer_popup_unconstrain(popup, leme_layer_full_box(popup->layer));
}

static void leme_layer_popup_handle_destroy(struct wl_listener *listener,
                                            void *data) {
  struct leme_layer_popup *popup = wl_container_of(listener, popup, destroy);
...
static void leme_layer_popup_handle_new_popup(struct wl_listener *listener,
                                              void *data) {
...
static void leme_layer_handle_new_popup(struct wl_listener *listener,
                                        void *data) {
  struct leme_layer_surface *layer =
...
static void leme_layer_handle_destroy(struct wl_listener *listener,
                                      void *data) {
  struct leme_layer_surface *layer = wl_container_of(listener, layer, destroy);
...
static void leme_layer_handle_new_surface(struct wl_listener *listener,
                                          void *data) {
  struct leme_server *server =
  ...
  if (wlr_layer_surface->output != NULL) {
    output = leme_output_from_wlr_output(server, wlr_layer_surface->output);
  }
